...
public class InstanceNetworkingStepDefinitions extends InstanceManagementStepDefinitionBase {
    ...
    /**
     * Simulates connection loss of an instance.
     * 
     * @param instanceId the node that is to be disconnected.
     */
    @When("instance {string} loses connection")
    public void whenConnectionLoss(String node){
    	executionContext.setKeepDiverting(true);
    	int ssHportToDisconnect = executionContext.getInstanceFromId(node).getServerPort("ssh", 0);
    	try {
        	this.divertTraffic(ssHportToDisconnect);	    
    	}
    	catch (Exception e) {
    		log.debug(e);
    	}
    ...
    @When("instances re-establish connection")
    public void reestablishConnections() {
    	executionContext.setKeepDiverting(false);
    }

    
    /**
     * Utilizes WinDivert to cut connection to ports.
     * 
     * @param port the port that is to be disconnected.
     */
	private void divertTraffic(int port) throws Exception {
		new Thread(() -> {
			try {
				WinDivert w = new WinDivert("tcp.DstPort == " + port + " or tcp.SrcPort == " + port);
		        w = w.open(); // Open the WinDivert handle
		        while (executionContext.getKeepDiverting()) {
		            w.recv(); // Intercept packets (but don't re-inject) 
	        	}
			}
			catch(Exception e) {
			  log.debug("Cannot divert packets!");
			}
